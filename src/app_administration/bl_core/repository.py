...
Les transactions métier et les métadonnées sont stockées dans Lakebase
PostgreSQL. Les binaires documentaires sont externalisés dans un Volume Unity
Catalog en production. Toutes les mutations critiques sont atomiques, auditées
et protégées par verrouillage optimiste.
"""
...
from .storage import volume_store
...
def enregistrer_notification(type_notif: str, numero_bl: str, message: str,
    ...
    """Crée un événement et ses livraisons par canal dans une même transaction.

    La clé d'idempotence empêche un double clic ou un rejeu de produire deux
    alertes. Les canaux sont configurés dans ``notification_canaux``.
    """
    s = get_settings()
    key = idempotency_key or hashlib.sha256(
        f"{type_notif}|{numero_bl.strip().upper()}|{message}".encode("utf-8")
    ).hexdigest()
    with database.transaction() as tx:
        event = tx.fetch_one(
            f"INSERT INTO {s.pg_schema}.notifications "
            "(event_key, type_notif, numero_bl, message, cree_par) "
            "VALUES (%(key)s, %(t)s, %(num)s, %(msg)s, %(par)s) "
            "ON CONFLICT (event_key) DO UPDATE SET event_key = EXCLUDED.event_key "
            "RETURNING id",
            {"key": key, "t": type_notif, "num": numero_bl, "msg": message, "par": utilisateur},
        )
        event_id = int(event["id"])
        tx.execute(
            f"INSERT INTO {s.pg_schema}.notification_livraisons "
            "(notification_id, canal, statut, prochaine_tentative_le) "
            f"SELECT %(id)s, code, 'EN_ATTENTE', now() FROM {s.pg_schema}.notification_canaux "
            "WHERE actif = true ON CONFLICT (notification_id, canal) DO NOTHING",
            {"id": event_id},
        )
...
def lister_notifications(limite: int = 200) -> pd.DataFrame:
    s = get_settings()
    return _run(
        f"SELECT n.cree_le, n.type_notif, n.numero_bl, n.message, n.cree_par, "
        "COALESCE(bool_and(l.statut = 'ENVOYEE'), false) AS envoyee, "
        "string_agg(l.canal || ':' || l.statut, ', ' ORDER BY l.canal) AS livraisons "
        f"FROM {s.pg_schema}.notifications n "
        f"LEFT JOIN {s.pg_schema}.notification_livraisons l ON l.notification_id = n.id "
        "GROUP BY n.id ORDER BY n.cree_le DESC LIMIT %(lim)s",
...
def enregistrer_page(id_bl: str, index_page: int, image_bytes: bytes) -> None:
    """Enregistre une page de manière idempotente.

    En production, le binaire va dans un volume Unity Catalog et Lakebase ne
    conserve que les métadonnées. Si l'écriture SQL échoue, le fichier nouvellement
    créé est supprimé par compensation.
    """
    s = get_settings()
    ...
    stored = None
    if settings.image_backend == "volume":
        stored = volume_store().put(id_bl, index_page, id_photo, image_bytes)
    try:
        with database.transaction() as tx:
            inserted = tx.execute(
                f"INSERT INTO {s.pg_schema}.pieces_jointes_bl "
                "(id_photo, id_bl, contenu, storage_uri, sha256, taille_octets, "
                "content_type, index_page) "
                "VALUES (%(idp)s, %(idb)s, %(contenu)s, %(uri)s, %(sha)s, %(taille)s, "
                "'image/jpeg', %(idx)s) ON CONFLICT (id_bl, index_page) DO NOTHING",
                {
                    "idp": id_photo,
                    "idb": id_bl,
                    "contenu": image_bytes if settings.image_backend == "database" else None,
                    "uri": stored.uri if stored else None,
                    "sha": stored.sha256 if stored else hashlib.sha256(image_bytes).hexdigest(),
                    "taille": len(image_bytes),
                    "idx": index_page,
                },
            )
        if inserted == 0 and stored is not None:
            volume_store().delete(stored.uri)
    except Exception:
        if stored is not None:
            try:
                volume_store().delete(stored.uri)
            except Exception:
                logger.exception("Échec de compensation pour %s", stored.uri)
        raise
...
def telecharger_photo(id_photo: str) -> bytes:
    ...
    s = get_settings()
    df = _run(
        f"SELECT contenu, storage_uri, sha256 FROM {s.pg_schema}.pieces_jointes_bl "
        "WHERE id_photo = %(id)s",
        params={"id": id_photo},
        fetch=True,
    )
    ...
    if row.get("storage_uri"):
        contents = volume_store().get(str(row["storage_uri"]))
    elif row.get("contenu") is not None:
