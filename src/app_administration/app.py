...
from bl_core import notifications, pdf_bl, rbac, repository, teams, ui
...
# =====================================================================
# DIAGNOSTIC DES MENTIONS TEAMS
# =====================================================================
@st.dialog("🧪 Tester les mentions Teams", width="medium")
def dialog_test_mentions():
    """Publie dans le canal une carte mentionnant la même personne avec
    plusieurs formes d'identifiant : celle qui devient cliquable est la
    bonne à saisir dans le référentiel."""
    st.caption("Teams résout l'identifiant d'une mention différemment selon "
               "les tenants (comptes invités/externes en particulier). Cette "
               "carte de test les essaie toutes en une seule publication.")
    nom = st.text_input("Nom affiché", value="", placeholder="Younes El Hachi").strip()
    email = st.text_input("E-mail Microsoft 365", value="",
                          placeholder="prenom.nom@emotors.com").strip()
    teams_id = st.text_input("Teams ID (AAD Object ID)", value="",
                             placeholder="35822834-280e-4c8b-…").strip()
    upn = st.text_input("UPN explicite (si différent de l'e-mail)", value="",
                        placeholder="prenom.nom@tenant.onmicrosoft.com").strip()
    if st.button("📨 Envoyer la carte de test", type="primary",
                 use_container_width=True,
                 disabled=not nom or not (email or teams_id or upn)):
        succes, detail = teams.envoyer_test_mentions(
            nom=nom, email=email, teams_id=teams_id, upn=upn)
        if succes:
            st.success("Carte envoyée. Ouvrez le canal Teams : la ligne dont "
                       "le nom est cliquable indique la forme d'identifiant à "
                       "utiliser. Reportez-la dans la grille ci-dessus.")
        else:
            st.error(f"Envoi impossible : {detail}")


def dialog_test_mentions_bouton() -> None:
    if st.button("🧪 Tester les mentions Teams", key="btn_test_mentions"):
        dialog_test_mentions()
...
def render_vue_courante() -> None:
    ...
    if vue == VUE_DASHBOARD:
        ...
        st.caption("Ces informations servent à @mentionner le gestionnaire dans "
                   ...
                   "portefeuille. **Nom affiché** = texte de la mention (à "
                   "défaut, le code). **Teams ID** (AAD Object ID) est utilisé "
                   "en priorité s'il est renseigné, sinon l'**e-mail** ; sans "
                   "aucun des deux, le gestionnaire est cité sans être notifié.")
        vue_referentiel(
            "gestionnaires",
            vue,
            config_colonnes={
                ...
                "teams_id": st.column_config.TextColumn(
                    "Teams ID (AAD Object ID)",
                    help="GUID Entra ID de la personne. Prioritaire sur "
                         "l'e-mail : à renseigner si la mention n'est pas "
                         "cliquable dans Teams."
                ),
            },
        )
        dialog_test_mentions_bouton()
