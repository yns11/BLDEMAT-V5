...
def arguments():
    ...
    parser.add_argument("--catalog-erp", required=True)
    parser.add_argument("--schema-erp", required=True)
    parser.add_argument("--catalog-staging", required=True)
    parser.add_argument("--schema-staging", required=True)
    ...
    parser.add_argument(
        ...
        choices=("true", "false"),
        default="false",
        help="Active l'adaptateur optionnel DespatchAdvice-Sales/salesordernum.",
    )
    ...
    for name in (
        ...
        if not IDENTIFIER.fullmatch(getattr(args, name)):
            parser.error(f"--{name.replace('_', '-')} contient un identifiant invalide")
    return args
